...
export function AllActivitiesList({ data }: IAllActivitiesProps) {
  const { classes } = useStyles()
  ...
  const { mutate: deleteAllActivities } = useDeleteCandidateById()
  ...
  const openModalForDelete = (AllActivities: TCandidate) => {
    openConfirmModal({
      ...
      onConfirm: () => {
        deleteAllActivities(AllActivities.uuid)
        console.log('delete')
        showNotification({
          title: 'AllActivities Deleted !!',
          // message: `${Employee.fname} has been deleted successfully.`,
          message: `AllActivities has been deleted successfully.`,
        })
      },
    })
  }
  ...
  return (
    <>
      <ListViewLayout
        ...
        createDrawerTitle="Add New Activities "
        isError={false}
        isLoading={false}
